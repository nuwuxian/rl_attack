...
# format: env name, traj_type, iter
dataset_template = '../expert_dataset/%s_%s_%d'

def generate_trojan_traj(model, save_path=None, env=None, n_timesteps=0,
...
def generate_expert_dataset(env_name, n_episodes=1000):
        ...
        # add flexible path
        generate_expert_traj(expert.act, dataset_template%(env_name.split('/')[1], 'expert', n_episodes), env, n_episodes=n_episodes)

def generate_dummy_dataset(env_name, n_episodes=1000):
        ...
        expert = make_trigger_agent(env_name, env.action_space)
        # add flexible path
        generate_expert_traj(expert.act, dataset_template%(env_name.split('/')[1], 'dummy', n_episodes), env, n_episodes=n_episodes)

def generate_trojan_dataset(env_name, n_episodes=1000, ratio=0.1):
        env = make_trojan_env(env_name, ratio)
        expert = make_victim_agent(env_name, env.observation_space, env.action_space)
        # add flexible path
        generate_trojan_traj(expert.act, dataset_template%(env_name.split('/')[1], 'trojan', n_episodes)+'_{:.0e}'.format(ratio), env, n_episodes=n_episodes)
...
if __name__ == '__main__':
        ...
        if args.expert_type == 'expert':
            generate_expert_dataset(env_name, args.n_episodes)
        elif args.expert_type == 'trojan':
            generate_trojan_dataset(env_name, n_episodes=args.n_episodes, ratio=args.ratio)
        elif args.expert_type == 'dummy':
            generate_dummy_dataset(env_name)
