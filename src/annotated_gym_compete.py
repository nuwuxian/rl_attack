...
def pretty_policy_type(env_name, short, is_victim, is_masked, policy_type, policy_path):
    ...
    elif policy_type == 'ppo2_adv':
        try:
            path_components = policy_path.split(osp.sep)
            experiment_root = osp.sep.join(path_components[:-4])
            cfg = read_sacred_config(experiment_root, 'train')
            print("###### [annotated_sym_complete.py]Config for ppo2_adv model",cfg)
            victim_path = cfg['victim_path']
        except (IndexError, FileNotFoundError):
            victim_path = ''
        friendly, code = 'Adversary', f'Adv(Hua){victim_path}'
...
class AnnotatedGymCompete(gym.Wrapper):
    ...
    def __init__(self, env, env_name, agent_a_type, agent_a_path, agent_b_type, agent_b_path,
        ...
        self.font = ImageFont.truetype(f'/home/wzg13/hua/arial.ttf', font_size)
        self.font_bold = ImageFont.truetype(f'/home/wzg13/hua/arial.ttf', font_size)
    ...
    def _render(self, mode='human', close=False):
        ...
        if mode == 'rgb_array':
            ...
            for k, label in texts.items():
                ...
                print("####### [annotated_sym_complete.py] ", self.env_name, self.short_labels, cur_agent)
