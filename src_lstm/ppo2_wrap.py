...
from policy import modeling_state, LSTMPolicy
...
class MyPPO2(ActorCriticRLModel):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    self.obs_opp_mean = tf.placeholder(dtype=tf.float32, shape=[train_model.obs_ph.shape[0]],
                                                       name='obs_opp_mean')
                    self.obs_opp_var = tf.placeholder(dtype=tf.float32, shape=[train_model.obs_ph.shape[0]],
                                                      name='obs_opp_var')
                    self.opp_state_ph = []
                    ...
                    self.state_modeling_mse = tf.reduce_mean(
                        tf.square(tf.math.maximum(tf.abs(obs_oppo_predict - self.obs_opp_next_ph) - 1e-2, 0)))
                    ...
                    # todo check the opp_action mse
                    self.grads = tf.gradients(self.change_opp_action_mse, action_opp_mal_noise)
    ...
    def _train_step(self, learning_rate, cliprange, obs, returns, masks, actions, values, neglogpacs,
                    o_opp, a_opp_next, o_opp_next, states_opp, attention, is_stochastic,
        ...
        o_opp_mean = np.mean(o_opp_next, axis=0)
        o_opp_var = np.std(o_opp_next, axis=0)

        o_opp_next = (o_opp_next - o_opp_mean) / (o_opp_var + 1e-8)
        o_opp = (o_opp - o_opp_mean) / (o_opp_var + 1e-8)
        ...
        td_map = {self.train_model.obs_ph: obs, self.action_ph: actions, self.advs_ph: advs, self.rewards_ph: returns,
                  ...
                  self.obs_opp_ph: o_opp, self.action_opp_next_ph: a_opp_next, self.obs_opp_next_ph: o_opp_next,
                  self.obs_opp_mean: o_opp_mean, self.obs_opp_var:o_opp_var,
    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=1, tb_log_name="PPO2",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            # todo save the (s_{t}, a_{t}) -> s_{t + 1}
            im_input = []
            im_out = []
            ...
            for update in range(1, nupdates + 1):
                ...
                if len(im_out) < 200 and update % 5 == 0:
                    # input, output
                    input = np.concatenate((obs_oppo, actions_oppo, actions), axis=1)
                    out = o_opp_next
                    im_input.append(input)
                    im_out.append(out)
                elif len(im_input) >= 200:
                    # todo list -> numpy
                    im_input = np.vstack(im_input)
                    im_out = np.vstack(im_out)

                    total_sz = im_input.shape[0]
                    input_sz = im_input.shape[1]
                    out_sz = im_out.shape[1]
                    # todo write into the file and saved it
                    try:
                        with h5py.File(os.path.join(self.model_saved_loc, 'feat.h5'), 'wb') as file_feat, \
                             h5py.File(os.path.join(self.model_saved_loc, 'label.h5'), 'wb') as file_label:
                            file_feat.create_dataset('feats', (total_sz, input_sz), dtype='float')
                            file_feat[...] = im_input
                            file_label.create_dataset('labels', (total_sz, out_sz), dtype='float')
                            file_label[...] = im_out
                        file_feat.close()
                        file_label.close()
                    except:
                ...
                if states is None:  # nonrecurrent version
                    ...
                    for epoch_num in range(self.noptepochs):
                        ...
                        for start in range(0, self.n_batch, batch_size):
                            ...
                            slices_hua = (arr[mbinds] for arr in (obs_oppo, a_opp_next, o_opp_next, states_oppo, attention))
