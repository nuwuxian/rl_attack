...
from pretrain_model import RL_func
...
class MyPPO2(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-4, vf_coef=0.5,
                 ...
                 model_saved_loc=None, env_name=None, env_path=None, rl_path=None, var_path=None):
        ...
        with open(self.var_path,'rb') as fid:
            [self.in_mean, self.in_var, self.out_mean, self.out_var] = pkl.load(fid)
        fid.close()
    ...
    def setup_model(self):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                # define a rl model
                with tf.variable_scope("pretrained_rl", reuse=False):
                    self.rl_model = RL_func(self.observation_space.shape[0] + 2 * self.action_space.shape[0], \
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    # Xian added
                    self.action_opp_ph = tf.placeholder(dtype=tf.float32, shape=self.action_ph.shape,
                                                        name="action_opp_ph")
                    self.action_opp_next_ph = tf.placeholder(dtype=tf.float32, shape=self.action_ph.shape,
                                                             name="action_opp_next_ph")
                    # both normalized
                    self.obs_opp_ph = tf.placeholder(dtype=tf.float32, shape=train_model.obs_ph.shape,
                                                     name="obs_opp_ph")
                    self.obs_opp_next_ph = tf.placeholder(dtype=tf.float32, shape=train_model.obs_ph.shape,
                                                          name="obs_opp_next_ph")
                    # add stochastic_ph
                    # define the new mean and var
                    self.stochastic_ph = tf.placeholder(tf.bool, [], name="stochastic_ph")
                    self.in_mean_ph = tf.placeholder(dtype=tf.float32, shape=[train_model.obs_ph.shape[0] + \
                                                  2 * self.action_ph.shape[0]], name='in_mean')
                    self.in_var_ph = tf.placeholder(dtype=tf.float32, shape=[train_model.obs_ph.shape[0] + \
                                                  2 * self.action_ph.shape[0]], name='in_var')

                    self.out_mean_ph = tf.placeholder(dtype=tf.float32, shape=[train_model.obs_ph.shape[0]],
                                                   name='out_mean')
                    self.out_var_ph = tf.placeholder(dtype=tf.float32, shape=[train_model.obs_ph.shape[0]],
                                                   name='out_var')

                    self.opp_state_ph = []
                    self.opp_state_ph.append(tf.placeholder(tf.float32, [None, 128], name="lstmv_c"))
                    self.opp_state_ph.append(tf.placeholder(tf.float32, [None, 128], name="lstmv_h"))
                    self.opp_state_ph.append(tf.placeholder(tf.float32, [None, 128], name="lstmp_c"))
                    self.opp_state_ph.append(tf.placeholder(tf.float32, [None, 128], name="lstmp_h"))

                    action_ph_noise = train_model.deterministic_action
                    # check the state transition loss
                    input = tf.concat([self.obs_opp_ph, self.action_opp_ph, self.action_ph], 1)
                    input = tf.divide(tf.subtract(input, self.in_mean_ph), self.in_var_ph + 1e-8)
                    obs_oppo_predict = self.rl_model(input)
                    
                    noise_input = tf.concat([self.obs_opp_ph, self.action_opp_ph, action_ph_noise], -1)
                    noise_input = tf.divide(tf.subtract(noise_input, self.in_mean_ph), self.in_var_ph + 1e-8)
                    obs_oppo_noise_predict = self.rl_model(noise_input)
                    obs_oppo_noise_predict = tf.add(tf.multiply(obs_oppo_noise_predict, self.out_var_ph), \
                                                    self.out_mean_ph)
                    '''
                    with tf.variable_scope("statem", reuse=True):
                        obs_oppo_predict, obs_oppo_noise_predict = modeling_state(self.action_ph, action_ph_noise,
                                                                                   self.obs_opp_ph)
                        obs_oppo_noise_predict = tf.add(tf.multiply(obs_oppo_noise_predict, self.obs_opp_var), \
                                                        self.obs_opp_mean)
                    '''
                    ...
                    # re-normalize the obs_oppo_noise_predict
                    if not self.black_box_att:
                        # reshape the state into lstm
                        obs_oppo_noise_predict = tf.reshape(obs_oppo_noise_predict, \
                                                                    [-1, 1, obs_oppo_noise_predict.shape[1]])
                        # change into the function
                        with tf.variable_scope('victim', reuse=False):
                            action_opp_mal_noise = LSTMPolicy(obs_oppo_noise_predict, self.stochastic_ph,
                                                              self.opp_state_ph, self.observation_space,
                                                              self.action_space, [128, 128], False, True)
                            # reshape the dimiension
                            action_opp_mal_noise = tf.squeeze(action_opp_mal_noise, axis=1)
                    else:
                        pass
                    self.opp_state_ph = tuple(self.opp_state_ph)
                    ...
                    # self.obs_opp_next_ph is normalized
                    self.state_modeling_mse = tf.reduce_mean(
                        tf.square(obs_oppo_predict - self.obs_opp_next_ph))
                    ...
                    if not self.black_box_att:
                        self.params = [params, tf_util.get_trainable_vars("pretrained_rl")]
                    else:
                        pass
                ...
                if not self.black_box_att:
                    victim_variable = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, "loss/victim")
                    param = load_from_file(param_pkl_path=self.env_path)
                    setFromFlat(victim_variable, param, sess=self.sess)
                # load the pretrained transition model
                if self.rl_path != None:
                    t_variable = tf_util.get_trainable_vars("pretrained_rl")
                    param = load_from_file(param_pkl_path=self.rl_path)
                    setFromFlat(t_variable, param, sess=self.sess)
                self.summary = tf.summary.merge_all()

    def _train_step(self, learning_rate, cliprange, obs, returns, masks, actions, values, neglogpacs,
                    o_opp, a_opp, a_opp_next, o_opp_next, states_opp, attention, is_stochastic,
        ...
        o_opp_mean = self.out_mean
        o_opp_var = self.out_var
        in_mean = self.in_mean
        in_var = self.in_var

        o_opp_next = (o_opp_next - o_opp_mean) / (o_opp_var + 1e-8)
        ...
        td_map = {self.train_model.obs_ph: obs, self.action_ph: actions, self.advs_ph: advs, self.rewards_ph: returns,
                  ...
                  self.obs_opp_ph: o_opp, self.action_opp_ph: a_opp, self.action_opp_next_ph: a_opp_next,
                  self.obs_opp_next_ph: o_opp_next, self.in_mean_ph: in_mean, self.in_var_ph: in_var, 
                  self.out_mean_ph: o_opp_mean, self.out_var_ph:o_opp_var, self.attention: attention
                  }

        if states is not None:
            td_map[self.train_model.states_ph] = states
            td_map[self.train_model.dones_ph] = masks
        # states_opp : batch_size * num_env * rnn_dim
        # reshape to num_env * batch_size * rnn_dim
        # Add by Xian
        states_opp = np.swapaxes(states_opp, 0, 1)
        # Given a default value
        # td_map[self.victim_model.observation_ph] = np.zeros((states_opp.shape[1], 1, 384))
        td_map[self.opp_state_ph] = list(states_opp)
        td_map[self.stochastic_ph] = is_stochastic
        ...
        if writer is not None:
            ...
            if self.full_tensorboard_log and (1 + update) % 10 == 0:
                ...
                summary, policy_loss, value_loss, policy_entropy, approxkl, clipfrac, \
    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=1, tb_log_name="PPO2",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, nupdates + 1):
                ...
                obs_oppo, actions_oppo, states_oppo, o_next, o_opp_next, a_opp_next = runner.run()
                ...
                if states is None:  # nonrecurrent version
                    ...
                    for epoch_num in range(self.noptepochs):
                        ...
                        for start in range(0, self.n_batch, batch_size):
                            ...
                            slices_hua = (arr[mbinds] for arr in (obs_oppo, actions_oppo, a_opp_next, o_opp_next, states_oppo, attention))
...
class Runner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        for _ in range(self.n_steps):
            ...
            mb_states_oppo.append(np.asarray(self.env.get_attr('oppo_state')))
            # debug for the mb_state dimension
            for info in infos:
        ...
        mb_states_oppo = np.asarray(mb_states_oppo)
        ...
        mb_obs_oppo, mb_actions_oppo, mb_states_oppo, mb_o_next, mb_o_opp_next, mb_a_opp_next = \
