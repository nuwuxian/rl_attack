...
class SharedMemoryPlayerAgent:
    ...
    """
    def __init__(self, scene, game_server_guid, player_n, profix=""):
        """
    ...
    def read_env_id_and_create_env(self, max_episode_steps):
        ...
        if self.player_n == 0:
            ...
            self.env = Monitor(self.env.unwrapped, log_dir, allow_early_resets=True,
                               max_episode_steps=max_episode_steps, info_keywords=('score_board',))
...
class SharedMemoryServer:
    ...
    """
    def __init__(self, scene, game_server_guid, want_test_window, profix=""):
        self.scene = scene
        self.plist = []
    ...
    def serve_forever(self, max_episode_steps=1000):
        for p in self.plist:
            p.read_env_id_and_create_env(max_episode_steps)
